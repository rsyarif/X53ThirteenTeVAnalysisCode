...
void ElectronStudy(){
  ...
  for(unsigned int i=0; i < chirals.size();i++){
	...
	for(unsigned int k=0;k<(*mvas).size();k++){
	  ...
	  if(fabs(eta) < 0.8){
	    ...
	    if(mva80X>0.017) h_num_mvaJulie->Fill(pt);
	  }
  ...
  g_mva_rh->SetLineColor(kRed);
  g_mva_rh->Draw("plsame");
  g_mva80X_rh->SetLineColor(kRed);
  g_mva80X_rh->SetLineStyle(2);
  g_mva80X_rh->Draw("plsame");
  g_mvaJulie_rh->SetLineColor(kRed);
  g_mvaJulie_rh->SetLineStyle(4);
  g_mvaJulie_rh->Draw("plsame");

  leg->AddEntry(g_mva_lh,"LH - 74X MVA","l");
  leg->AddEntry(g_mva80X_lh,"LH - 80X MVA Nominal WP","l");
  leg->AddEntry(g_mvaJulie_lh,"LH - 80X MVA Julie WP","l");
  leg->AddEntry(g_mva_rh,"RH - 74X MVA","l");
  leg->AddEntry(g_mva80X_rh,"RH - 80X MVA Nominal WP","l");
  leg->AddEntry(g_mvaJulie_rh,"RH - 80X MVA Julie WP","l");
  leg->Draw("same");
