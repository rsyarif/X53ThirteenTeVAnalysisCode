...
int getEtaBin(TLepton* el){
  int bin =-1;
  float eta = fabs(el->eta);
  if(eta<0.2) bin=1;
  else if(eta<0.6) bin=2;
  else if(eta<1.0) bin=3;
  else if(eta<1.4) bin=4;
  else if(eta<1.5) bin=5;
  else if(eta<2.2) bin=6;
  else if(eta<2.4) bin=7;
  return bin;
...
int main(int argc, char* argv[]){
  ...
  else  if(argv1=="Data" && argv2=="25ns") {filename="root://cmseos.fnal.gov//store/user/lpctlbsm/clint/Run2016B/June18/ljmet_trees/ljmet_Data_ElEl.root"; data=true; FiftyNS=false;}
  ...
  else if(argv1=="MC" && argv2=="25ns") {filename="/eos/uscms/store/user/lpctlbsm/clint/Spring15/25ns/Nov17/ljmet_trees/ljmet_DYJets.root"; data=false; FiftyNS=false;}
  else{
    std::cout<<"Need to specify whether running on Data or MC and 25 or 50ns. The four possible ways of running are\n"
  ...
  std::string outname;
  if(data)outname="ChargeMisID_Data_Run2015D_Electrons_"+ID+"_CrossCheck.root"; 
  else outname="ChargeMisID_MC_Electrons_"+ID+"_CrossCheck.root"; 
  ...
  for(int ient=0; ient<nEntries; ient++){
    ...
    //skip events where the electrons are not in same abseta bin
    int bin1 = getEtaBin(lep1);
    int bin2 = getEtaBin(lep2);
