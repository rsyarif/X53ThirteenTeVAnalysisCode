...
void DrawAndSaveSingleEra(Variable* var, std::vector<Sample*> vBkg, std::vector<Sample*> vSig,Sample* dataSample, TFile* outfile, std::string elID, std::string muID, int nMu, int cutIndex,std::string weightstring,std::string era){
  ...
  std::stringstream cutstring;
  if(nMu>=0){
    if(cutIndex==0){cutstring<<"("<<weightstring<<"( !(Channel==2 && TMath::Abs(Lep1Phi - Lep2Phi) < 1.25 && ( (Lep1Eta>1.2 && Lep2Eta >1.2) || (Lep1Eta<-1.2 && Lep2Eta <-1.2))) && (Channel=="<<nMu<<") ))";}
  ...
  cmstex->DrawLatex(0.15,0.96,"CMS Preliminary");
  if(era=="2016B-D"){
    lumitex->DrawLatex(0.65,0.96,"17.7 fb^{-1} (13 TeV)");
  }
  else{
    lumitex->DrawLatex(0.65,0.96,"18.2 fb^{-1} (13 TeV)");
  }
  //draw latex for channels
  TLatex* chantex = new TLatex();
  chantex->SetNDC();
  ...
  if(cutIndex==3) cutname =  "nConst2to4";
  if(cutIndex==4) cutname =  "nConst5plus";
  std::string pdfname = "./plots_forPAS/"+(var->name)+"_"+(vBkg[0]->cutname)+"_"+channel+"_"+cutname+"_Mu"+muID+"_El"+elID+"_"+era+".pdf";
  std::string pngname = "./plots_forPAS/"+(var->name)+"_"+(vBkg[0]->cutname)+"_"+channel+"_"+cutname+"_Mu"+muID+"_El"+elID+"_"+era+".png";
