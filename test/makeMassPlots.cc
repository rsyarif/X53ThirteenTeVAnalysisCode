...
//eaxample usage:
//root -b -q -l 'makeMassPlots.cc("MVATightRC","CBTight")' 
...
void DrawNConstComparison(Sample* s){
  TTree* t = s->tree;
  TH2F* h = new TH2F("h",";nConstituents(boosted) - nConstituents;nConstituents",10,0,10,10,0,10);
...
void DrawHTComparison(Sample* s){
  TTree* t = s->tree;
  TH2F* h = new TH2F("h",";H^{lep}_{T}(boosted);H^{lep}_{T}",25,0,3000,25,0,3000);
...
void makeMassPlots(std::string elID, std::string muID){

  //make output file
  TFile* fout = new TFile("plots_forBoostedAnalysis.root","RECREATE");
  ...
  std::vector<Variable*> vVariables = getVariableVecForBoosted();
  ...
  for(int j=2; j <3; j++){
    for(std::vector<Variable*>::size_type i=0; i<vVariables.size();i++){
      //if(vVariables.at(i)->name.find("X53Mass")==std::string::npos) continue;
  ...
  for(size_t i=0; i<vBkgSamples.size();i++){
    DrawNConstComparison(vBkgSamples.at(i));
    DrawHTComparison(vBkgSamples.at(i));
  }
  for(size_t i=0; i<vSigSamples.size();i++){
    DrawNConstComparison(vSigSamples.at(i));
    DrawHTComparison(vSigSamples.at(i));
  }
  DrawNConstComparison(dataSample);
  DrawHTComparison(dataSample);
...
void DrawAndSave(Variable* var, std::vector<Sample*> vBkg, std::vector<Sample*> vSig,Sample* dataSample, TFile* outfile, std::string elID, std::string muID, int nMu, int cutIndex){
 ...
 c1->SetLogy();
  ...
  for(std::vector<Sample*>::size_type uk=0; uk<vBkg.size(); uk++){
    ...
    Sample* s = vBkg.at(uk);
    //std::cout<<"got sample "<<s->name<<std::endl;
    //s->setHist(var,(cutstring.str()).c_str());
    //std::cout<<"drawing for sample: "<<s->name<<std::endl;
    TH1F* h = new TH1F("h",(var->name).c_str(), var->nbins, var->xmin, var->xmax);
 
    h->Sumw2();
    TTree* t = s->tree;
    //std::cout<<"projecting for sample: "<<s->name<<std::endl;
    t->Project("h",(var->name).c_str(),(cutstring.str()).c_str());
    //std::cout<<"projected for sample: "<<s->name<<std::endl;
	...
	if(h->GetBinContent(hbin+1)<0) h->SetBinContent(hbin+1,0);
      ...
      //}
      //std::cout<<"set bins to zero"<<std::endl;
    ...
    //std::cout<<"scaled sample"<<std::endl;
    ...
    //add to error hist
    h_err->Add(h);
    //std::cout<<"added to error hist"<<std::endl;
    //aesthetics
    ...
    //std::cout<<"added to summed bkg hist"<<std::endl;
  ...
  //std::cout<<"made ttX and multiboson hists and added to tstack"<<std::endl;
  h_cmid->SetFillColor(kAzure+6);
  tStack->Add(h_cmid);
  leg->AddEntry(h_cmid,"ChargeMisID","f");
  //std::cout<<"added cmid to tstack"<<std::endl;
  h_npf->SetFillColor(kGray);
  tStack->Add(h_npf);
  leg->AddEntry(h_npf, "NonPrompt", "f");
  
  //std::cout<<"making errors"<<std::endl;
  //make errors
  ...
  if(var->name.find("X53Mass")==std::string::npos) tStack->GetYaxis()->SetRangeUser(0.008,100000);
  if(var->name.find("X53Mass")==std::string::npos) tStack->SetMaximum(15 * tStack->GetMaximum()); 
  else tStack->SetMaximum(1.5 * tStack->GetMaximum());
  ...
  for(std::vector<Sample*>::size_type i1=0;i1<vSig.size();i1++){
    ...
    Sample* s = vSig.at(i1);
    if(s->name.find("700")==std::string::npos && (var->name.find("X53Mass")==std::string::npos)) continue;
    //std::cout<<" drawing for sample: "<<s->name<<std::endl;
    TH1F* h = new TH1F("h",(var->name).c_str(), var->nbins, var->xmin, var->xmax);
    TTree* t = s->tree;
  ...
  std::string pdfname = "./plots_forBoostedAna/"+(var->name)+"_"+(vBkg[0]->cutname)+"_"+channel+"_"+cutname+"_Mu"+muID+"_El"+elID+".pdf";
  std::string pngname = "./plots_forBosotedAna/"+(var->name)+"_"+(vBkg[0]->cutname)+"_"+channel+"_"+cutname+"_Mu"+muID+"_El"+elID+".png";
