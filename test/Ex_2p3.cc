...
void Ex_2p3(){
  ...
  //output file
  TFile* fbg = new TFile("bg_nonPrompt_8TeV.root","RECREATE");
  ...
  TH1F* leadlep_pt = new TH1F("leadlep_pt","leading lepton pt",50,0,500);
  TH1F* sublep_pt = new TH1F("sublep_pt","subleading lepton pt",50,0,500);
  ...
  TH1F* leadjet_ak8pt = new TH1F("leadjet_ak8pt","leading jet pt",100,0,1000);
  TH1F* subjet_ak8pt = new TH1F("subjet_ak8pt","sub-leading jet pt",100,0,1000);


  TH1F* met_h = new TH1F("met_h","MET",100,0,1000);
  TH1F* HT_h = new TH1F("HT_h","HT",250,0,2500);
  TH1F* HT_ak8_h = new TH1F("HT_ak8_h","HT",250,0,2500);
  ...
  tdata->SetBranchAddress("CA8JetPt_DileptonCalc",&jetak8Pts);
  ...
  for(int ient = 0; ient < nEntries; ient++){
    ...
    for(unsigned int ijet=0;ijet<jetPts->size();ijet++){
      if(maxpt<jetPts->at(ijet)) maxpt = jetPts->at(ijet);
    }
    ...
    for(unsigned int ijet=0; ijet<jetPts->size();ijet++){
      if(penpt<jetPts->at(ijet) && jetPts->at(ijet)!=maxpt) penpt=jetPts->at(ijet);
    }

    //check for second jet
    if(penpt < 30) continue;
    NJetpt2cut+=1;

    //check for second jet
    if(jetPts->at(1) < 30) continue;
    NJetpt2cut+=1;
    ...
    float ak8HT=0;
    for(unsigned int ijet = 0; ijet<jetak8Pts->size(); ijet++){
      ak8HT+=fabs(jetak8Pts->at(ijet));
    }
    ...
    vector <Lepton*> vLep;
    //vector <Electron*> vEl;
    //make vector for leptons passing 8 TeV cuts
    vector< Lepton*> v8TeVLep;
    ...
    for (unsigned int uiEl = 0; uiEl < elPts->size(); uiEl++){
      ...
      vLep.push_back(lep);
      //apply 8 TeV cuts:
      if( (fabs(lep->eta<2.4) && !( (1.4442 < fabs(lep->eta)) && (fabs(lep->eta<1.566))) ) && ( lep->pt >30) && (lep->Tight)) v8TeVLep.push_back(lep);
    ...
    for (unsigned int uiMu = 0; uiMu <muPts->size(); uiMu++){
      ...
      vLep.push_back(lep);
      //apply 8 tev cuts
      if( (lep->pt >30) && (fabs(lep->eta)<2.4) && (lep->Tight) ) v8TeVLep.push_back(lep);
    ...
    //same thing as above but for 8 tev cuts
    //vector for tight lepton requirements
    vector<Lepton*> vSS8TeVLep;
    bool samesign8TeV = false;
    for(unsigned int uiLep=0;uiLep<v8TeVLep.size();uiLep++){
      //all leptons are tight so just check same sign:
      int charge1 = vLep.at(uiLep)->charge;
      for(unsigned int ujLep=uiLep+1; ujLep<v8TeVLep.size();ujLep++){
	if( charge1== vLep.at(ujLep)->charge) samesign8TeV = true;
	if(samesign8TeV){
	  vSS8TeVLep.push_back(v8TeVLep.at(uiLep));
	  vSS8TeVLep.push_back(v8TeVLep.at(ujLep));
	}
      }
    ...
    for(unsigned int ijet=0;ijet<jetak8Pts->size();ijet++){
      if(maxak8pt<jetak8Pts->at(ijet)) maxak8pt = jetak8Pts->at(ijet);
    }

    float penak8pt=0;

    for(unsigned int ijet=0; ijet<jetak8Pts->size();ijet++){
      if(penak8pt<jetak8Pts->at(ijet) && jetak8Pts->at(ijet)!=maxak8pt) penak8pt=jetak8Pts->at(ijet);
    }

    std::cout<<"get max and min jet pts"<<std::endl;
    leadjet_ak8pt->Fill(maxak8pt);
    subjet_ak8pt->Fill(penak8pt);
    HT_ak8_h->Fill(ak8HT);


    int njet=0;
    for(unsigned int ijet = 0; ijet<jetPts->size(); ijet++){
      if(jetPts->at(ijet)>30) njet+=1;
    }
...
//now only events this far in loop have passed all cuts so plot HT
    //first check samesign leptons using 8 TeV cuts
    if(!samesign8TeV) continue;
    if(njet<3) continue;
