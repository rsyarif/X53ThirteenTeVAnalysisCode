...
//eaxample usage:
//root -b -q -l 'makePlots_forPaper.cc("MVATightRC","CBTightMiniIso")' 
...
TH1F* getPullPlot(TH1F* hData, THStack * h, Variable* var, TH1F* h_err, TH1F* h_np, TH1F* h_cmid);
      
void makePlots_forPaper(std::string elID, std::string muID){

  //make output file
  TFile* fout = new TFile("plots_forPaper.root","RECREATE");
  ...
  for(int j=2; j <3; j++){
    for(std::vector<Variable*>::size_type i=0; i<vVariables.size();i++){
      if( (vVariables.at(i))->name.find("cleanAK4HT")==std::string::npos ) continue;
...
void DrawAndSave(Variable* var, std::vector<Sample*> vBkg, std::vector<Sample*> vSig,Sample* dataSample, TFile* outfile, std::string elID, std::string muID, int nMu, int cutIndex){
  ...
  // only add to legend if not mumu channel
  if(nMu !=2) leg->AddEntry(h_cmid,"ChargeMisID","f");
  ...
  if(nMu==-1) tStack->SetMinimum(0.009);
  else tStack->SetMinimum(h_err->GetMinimum(0.0000000001));
  tStack->Draw("HIST");
  ...
  for(std::vector<Sample*>::size_type i1=0;i1<vSig.size();i1++){
    ...
    if(s->name.find("RH")==std::string::npos){
      leg->AddEntry(h,"X_{5/3}#bar{X}_{5/3} LH (0.7 TeV)","l");
    }
    else{
      leg->AddEntry(h,"X_{5/3}#bar{X}_{5/3} RH (0.7 TeV)","l");
    }
    //assert(h);
    //tStack->Add(h);
  ...
  //draw latex
  cmstex->DrawLatex(0.25,0.85,"CMS");
  //get info for lumi position
  float t = pad1->GetTopMargin();
  float r = pad1->GetRightMargin();
  lumitex->SetTextSize(0.6*t);
  lumitex->DrawLatex(1-r,1-t+0.2*t,"2.3 fb^{-1} (13 TeV)");

  //draw latex for channels
  TLatex* chantex = new TLatex();
  chantex->SetNDC();
  chantex->SetTextSize(0.06);
  if(nMu==-1) chantex->DrawLatex(0.25,0.77,"#bf{ee + e#mu + #mu#mu}");
  if(nMu==0) chantex->DrawLatex(0.25,0.77,"#bf{ee}");
  if(nMu==1) chantex->DrawLatex(0.25,0.77,"#bf{e#mu}");
  if(nMu==2) chantex->DrawLatex(0.25,0.77,"#bf{#mu#mu}");


  //now draw pull plot
  pad2->cd();
  TH1F* hpull = getPullPlot(hData, tStack, var, h_err,h_np,h_cmid);
  hpull->Draw("HIST");
  ...
  std::string pdfname = "./plots_forPaper/"+(var->name)+"_"+(vBkg[0]->cutname)+"_"+channel+"_"+cutname+"_Mu"+muID+"_El"+elID+".pdf";
  std::string pngname = "./plots_forPaper/"+(var->name)+"_"+(vBkg[0]->cutname)+"_"+channel+"_"+cutname+"_Mu"+muID+"_El"+elID+".png";
...
TH1F*
getPullPlot(TH1F* hData, THStack * h, Variable* var, TH1F* h_err, TH1F* h_np, TH1F* h_cmid){//Actually a residue plot for now
  ...
  hpull->SetTitle(";;#frac{(obs-bkg)}{uncertainty}");
  ...
  for(int i=1; i<=size; ++i){
	...
	error = pow(h_err->GetBinError(i),2) + pow(N-L,2);	
	float epoiss=0.0;
	//if(h_cmid->GetBinContent(i)==0) epoiss = epoiss + 1.8*1.8;
	//if(h_np->GetBinContent(i)==0) epoiss = epoiss + 1.8*1.8;
	error = pow( error + epoiss, 0.5); //add poisson errors if necessary
	diff = (hData->GetBinContent(i) - hsum->GetBinContent(i))/error;
      }
      else{
	error = pow(h_err->GetBinError(i),2) + pow(U-N,2);
	float epoiss=0.0;
	//if(h_cmid->GetBinContent(i)==0) epoiss = epoiss + 1.8*1.8;
	//if(h_np->GetBinContent(i)==0) epoiss = epoiss + 1.8*1.8;
	error = pow( error + epoiss, 0.5); //add poisson errors if necessary
  ...
  hpull->GetXaxis()->SetNdivisions(605);
